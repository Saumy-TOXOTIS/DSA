...
ll minimum(vll& array,ll n)
{
    ...
    while(low <= high)
    {
        ...
        {
            //Store minimum of sorted left part and reduce left part
            answer = min(array[low],answer);
            low = mid + 1;
        }
        //Check if Right part is Sorted
        else
        {
            //Store minimum of sorted right part and reduce right part
            answer = min(array[mid],answer);
            high = mid - 1;
        }
    }
    return answer;
...
TOXOTIS
{
    ...
    d_ll(n)
    d_llVecArr(a,n)//Rotated Sorted Array
    ll MIN = minimum(a,n);
    cout<<MIN<<endl;
}